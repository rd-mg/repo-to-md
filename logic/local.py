...
class LocalProcessor(BaseProcessor):
    def process(self, root_dir, output_file, extra_ignores=None, include_patterns=None, 
                style="markdown", split_size=None, header_text=None):
        """Processes a local directory using Repomix."""
        cmd = list(REPOMIX_CMD) + [root_dir, "--output", output_file, "--style", style]
        
        # Merge ignores
        if extra_ignores is not None:
            all_ignores = list(extra_ignores)
            cmd.extend(["--no-default-patterns", "--no-gitignore", "--no-dot-ignore"])
        else:
        ...
        self.log(f"Running Repomix: {' '.join(cmd)}")
        
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            stdout = result.stdout
            self.log("Repomix completed successfully.")
    ...
    def _count_total_words(self, output_file):
        ...
        # Pattern to find base.ext and base.1.ext, base.2.ext etc.
        # Repomix splits as: output.1.md, output.2.md
        patterns = [output_path.name, f"{base_name}.*{ext}"]
